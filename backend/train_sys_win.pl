...
path(From,To, _,X3, Length, Fare, 1):-
	...
	fare(From,To,Fare), count(From,To,Length).
		...
		% with linkage -> check discount for mrt
path(From, To,_, L3, Length, Fare, 1):-
	...
	((From \= A, To \= B) ->                     		
    					( 	(X == purple, Y == blue) ->  Discount = 14
                        ;   (X == blue, Y == purple) ->  Discount = 14
                        ;   (X == blue, Y == yellow) ->  Discount = 15
                        ;   (X == yellow, Y == blue) ->  Discount = 14
                        ;   (X == purple, Y == pink) ->  Discount = 15
                        ;   (X == pink, Y == purple) ->  Discount = 14
                        ;   Discount = 0)
    ;   Discount = 0),
    append(L2,[To],L3),
    Fare is F1 + F2 - Discount,
	count(From,A,C1), count(B,To,C2), Length is C1+C2.

		% no linkage: NO green found green
path(From,To,Visited,List,Length,Fare, Interchange):- 
    ...
    %;   true),
    ((From \= A, To \= B) ->                     		
    					( 	(X == purple, Z == blue) ->  Discount = 14
                        ;   (X == blue, Z == purple) ->  Discount = 14
                        ;   (X == blue, Z == yellow) ->  Discount = 15
                        ;   (X == yellow, Z == blue) ->  Discount = 14
                        ;   (X == purple, Z == pink) ->  Discount = 15
                        ;   (X == pink, Z == purple) ->  Discount = 14
                        ;   Discount = 0)
    ;   Discount = 0),
    L = [From],
...
% no linkage but green found green
path(From,To,Visited,List,Length,Fare,Interchange):-
    train(From, X), train(To, Y),
	(   	X == lightgreen -> Y \= darkgreen
	 ;   	X == darkgreen -> Y \= lightgreen
...
insert(compare_by_all, X, [Y|Ys], [Y|Zs]) :-
    ...
    insert(compare_by_all, X, Ys, Zs).
		...
		% choose path
% Sort a list by fare
compare_by_fare(Order, [_, Distance1, Fare1,Interchange1], [_, Distance2, Fare2,Interchange2]) :-
    (   Fare1 < Fare2 -> Order = '<'
    ;   Fare1 > Fare2 -> Order = '>'
...
compare_by_distance(Order, [_, Distance1, Fare1,_], [_, Distance2, Fare2,_]) :-
...
compare_by_time(Order, [_, Distance1, Fare1,Interchange1], [_, Distance2, Fare2,Interchange2]) :-
    (   (Distance1 + Interchange1) < (Distance2 + Interchange2) -> Order = '<'
    ;   (Distance1 + Interchange1) > (Distance2 + Interchange2) -> Order = '>'
    ;   (Fare1 > Fare2) 	-> Order = '>'
    ;   (Fare1 < Fare2) 	-> Order = '<'    
